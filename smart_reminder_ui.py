...
from data_analysis import get_recommended_interval
...
class SmartReminderWindow(QMainWindow):
    def __init__(self, app):
        ...
        analyze_button.setGeometry(160, 230, 101, 31)
        ...
        """)
        analyze_button.setText("分析数据")
        analyze_button.clicked.connect(self.analyze_flower)
        
        # 添加保存按钮
        save_button = QPushButton(frame)
        save_button.setGeometry(280, 230, 101, 31)
    ...
    def analyze_flower(self):
        ...
        if flower_name:
            # 获取推荐间隔天数
            interval = get_recommended_interval(flower_name)
